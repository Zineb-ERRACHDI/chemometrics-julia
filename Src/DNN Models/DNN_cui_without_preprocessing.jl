...
K_INIT =Flux.kaiming_normal(MersenneTwister(1);gain=sqrt(2))#the initialisation function
model = Chain(
...
# Loss graph
plot([loss_train, loss_test], label=["Train Loss" "Test Loss"], xlabel="epch", ylabel="Loss", title="Model loss", lw=2)


#dnn Coefficients
dnn= vec(model(xtestst'))
x_subset_dnn=xtestst[1:100, :]
y_subset_dnn=dnn[1:100, :]
