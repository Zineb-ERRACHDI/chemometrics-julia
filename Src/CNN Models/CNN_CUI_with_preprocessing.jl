...
for epoch in epochs
    @info "Epoch $epoch"
    Flux.train!(lossR, param, data, opt ,cb = throttle(() -> println(lossR(Xtrainst',ytrain)), 1))
...
plotxy( ytest,vec(pred); resolution = (500, 400),
    ...
    title="Predictions based on CNN model").f  

#cnn Coefficients
cnn= vec(model(xtestst'))
x_subset_cnn=xtestst[1:100, :]#substract the first 100 spectra
y_subset_cnn=cnn[1:100, :]#substract the first 100 predicted
...
##Visualize the type of preprocessing done by CNN layer
#the preprocessing of the convolution layer
conv_layer = model[1:2] 
conv_outputs = permutedims(Flux.flatten(conv_layer(xtestst')))
plotsp(conv_outputs, wl_num; nsamp = 100,
    xlabel = "Wavelength (nm)", ylabel = "X",
    title="Preprocessed train set spectra: 1D convolution").f

#the preprocessing of Savitzky-Golay
x=savgol(xtestst;f=3,pol=2,d=1)
plotsp(x, wl_num; nsamp = 100,
    xlabel = "Wavelength (nm)", ylabel = "X",
    title="Preprocessed train set spectra: Savitzky-Golay").f


#saving the trained model
BSON.@save "cnn_spectrochimie.bson" model